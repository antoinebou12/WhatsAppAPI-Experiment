...
def onIncomingMessageReceived(body):
   ...
   print(
       f'{idMessage}: At {str(eventDate)} Incoming from {json.dumps(senderData, ensure_ascii=False)} message = {json.dumps(messageData, ensure_ascii=False)}'
   )
...
def onIncomingCall(body):
   ...
   eventDate = datetime.fromtimestamp(body['timestamp'])
   fromWho = body['from']
   print(f'{idMessage}: Call from {fromWho} at {str(eventDate)}')

def onDeviceInfo(body):
   eventDate = datetime.fromtimestamp(body['timestamp'])
   deviceData = body['deviceData']
   print(f'At {str(eventDate)}: {json.dumps(deviceData, ensure_ascii=False)}')
...
def onOutgoingMessageReceived(body):
   ...
   messageData = body['messageData']
   print(
       f'{idMessage}: At {str(eventDate)} Outgoing from {json.dumps(senderData, ensure_ascii=False)} message = {json.dumps(messageData, ensure_ascii=False)}'
   )
...
def onOutgoingAPIMessageReceived(body):
   ...
   messageData = body['messageData']
   print(
       f'{idMessage}: At {str(eventDate)} API outgoing from {json.dumps(senderData, ensure_ascii=False)} message = {json.dumps(messageData, ensure_ascii=False)}'
   )
...
def onOutgoingMessageStatus(body):
   ...
   status = body['status']
   eventDate = datetime.fromtimestamp(body['timestamp'])
   print(f'{idMessage}: At {str(eventDate)} status = {status}')

def onStateInstanceChanged(body):
   eventDate = datetime.fromtimestamp(body['timestamp'])
   stateInstance = body['stateInstance']
   print(
       f'At {str(eventDate)} state instance = {json.dumps(stateInstance, ensure_ascii=False)}'
   )
...
def onStatusInstanceChanged(body):
   ...
   print(
       f'At {str(eventDate)} status instance = {json.dumps(statusInstance, ensure_ascii=False)}'
   )
